...
A log line becomes a span when both a trace id and a span id are present. Parent links use `parent_span_id` (or equivalent). Recognized fields include:

| Role | Keys (case-insensitive) |
|------|-------------------------|
| Trace id | `trace_id`, `traceId`, `dd.trace_id`, `otel.trace_id`, nested `dd.trace_id` |
| Span id | `span_id`, `spanId`, `dd.span_id`, `otel.span_id`, nested `dd.span_id` |
| Parent | `parent_span_id`, `parent_id`, `dd.parent_id`, … |
