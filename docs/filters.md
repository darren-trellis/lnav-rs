...
`s` / `:view sidebar toggle` shows a list of filters, then manually hidden lines (`·N` with a preview). File settings live under `[view]`: `sidebar.position` (`left` or `right`, default `right`), `sidebar.width` (default 28; clamped to leave room for the list), and `sidebar.scrollbar.vertical` / `sidebar.scrollbar.horizontal` (same idea as `main.scrollbar.*`). Set via `:config set view.sidebar.width`, `view.sidebar.position`, etc.
