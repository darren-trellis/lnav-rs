...
With details focused, `/` searches inside the overlay (`n`/`N` cycle matches). Nested JSON fields render as a tree when `[details] json_tree` is on (`tab_width` sets indent per level). Overlay height grows with content up to `max_height` (and screen space). If opening or resizing details would clip the selected log, the list scrolls just enough to keep that line visible (on the last list row, above details). `:config set` still uses the flat names (`details_json_tree`, `details_tab_width`, `details_max_height`, …).
