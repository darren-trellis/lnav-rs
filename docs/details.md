...
With details focused, `/` searches inside the overlay (`n`/`N` cycle matches). Nested JSON fields render as a tree when `[view.details] json_tree` is on (`tab_width` sets indent per level). With `wrap` off, `h`/`l` (or ←/→) scroll horizontally and `[view.details.scrollbar] horizontal` shows a bottom bar. Overlay height grows with content up to `max_height` (and screen space). If opening or resizing details would clip the selected log, the list scrolls just enough to keep that line visible (on the last list row, above details). Set via `:config set view.details.json_tree`, `view.details.tab_width`, `view.details.max_height`, etc.
